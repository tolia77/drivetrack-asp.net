    ...
    Optional<DateOnly?> NextMaintenanceDate)
{
    ...
    internal UpdateVehicleCommand MergedOnto(Vehicle vehicle)
    {
        ...
        return new UpdateVehicleCommand(
            Optional<string>.Of(Model.Or(vehicle.Model)),
            Optional<string>.Of(LicensePlate.Or(vehicle.LicensePlate)),
            Optional<decimal>.Of(CapacityKg.Or(vehicle.CapacityKg)),
            Optional<int>.Of(Mileage.Or(vehicle.Mileage)),
            Optional<DateOnly?>.Of(NextMaintenanceDate.Or(vehicle.NextMaintenanceDate)));
    }
}
