    ...
    Optional<int?> VehicleId)
{
    ...
    internal UpdateDriverCommand MergedOnto(Driver driver, UserAccount account)
    {
        ...
        return new UpdateDriverCommand(
            Optional<string>.Of(FirstName.Or(account.FirstName)),
            Optional<string>.Of(LastName.Or(account.LastName)),
            Optional<string>.Of(LicenseNumber.Or(driver.LicenseNumber)),
            Optional<int?>.Of(VehicleId.Or(driver.VehicleId)));
    }
}
