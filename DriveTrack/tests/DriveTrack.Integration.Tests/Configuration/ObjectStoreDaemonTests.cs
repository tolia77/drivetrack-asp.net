...
/// the same <c>AddInfrastructure</c>, pointed at a real Garage node that <c>compose.yaml</c>'s own
/// <c>objects-init</c> script provisioned.
/// <para>
...
public class ObjectStoreDaemonTests(GarageFixture garage) : IDisposable
{
    ...
    {
        // compose.yaml says objects-init creates this bucket. Nothing checked that it did.
        var cancellationToken = TestContext.Current.CancellationToken;
    ...
    public async Task Provisioning_the_node_a_second_time_succeeds_and_changes_nothing()
    {
        ...
        Assert.True(
            run.ExitCode == 0,
            $"Re-running compose.yaml's objects-init script exited {run.ExitCode}:"
                + Environment.NewLine
                + run.Logs);
    ...
    public async Task The_shell_flags_compose_runs_the_provisioning_under_stop_it_at_the_first_failure()
    {
        ...
        // they do, and a healthy provisioning run never fails a step, so `-e` could be dropped from
        // compose.yaml with every other case here still green. What that would cost: a step that
        // failed halfway runs on and still exits 0, and the app service's
    ...
    /// <summary>
    /// The permission triple <c>compose.yaml</c> grants: read and write, and deliberately not owner.
    /// </summary>
